...
public class SemanticDuration extends AbstractNumericInterpreter {
	/**
	 * 
	 */
	...
	static public enum EDurationUnits implements ISingleUnit {
		...
		undef(0), s(1 / 60d), min(1d), h(60d), day(60 * 24d), week(60 * 24 * 7d), month(
				60 * 24 * (365d / 12d)), year(60 * 24 * 365d);
	...
	private SemanticDuration(String surfaceForm, EDurationUnits unit, double meanValue, boolean about, double fromValue,
			double toValue) {
		super(surfaceForm);
		this.unit = unit;
	...
	public SemanticDuration convertTo(EDurationUnits toWeightUnit) {
		return new SemanticDuration(surfaceForm, toWeightUnit, convertValue(meanValue, toWeightUnit), about,
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SemanticDuration other = (SemanticDuration) obj;
	...
	public SemanticDuration normalize() {
	...
	((~|\babout[^\d\w\.,])?)(((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))?)|((((\d+(\.|,)\d+)|\d+))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?)))|((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))))\b
		...
		private EDurationUnits unit = defaultUnit;
		...
		public Builder setUnit(EDurationUnits unit) {
			this.unit = unit;
			return this;
		}
		...
		public SemanticDuration build() {
			...
			return new SemanticDuration(surfaceForm, unit, _meanValue, about, _fromValue, _toValue);
		}

		public Builder interprete(final String surfaceForm) {

			Matcher matcher = SemanticDuration.PATTERN.matcher(surfaceForm);
		...
		public Builder fromMatcher(final Matcher matcher) {
			...
			about = matcher.group(SemanticDuration.aboutGroupName) != null
					&& !matcher.group(SemanticDuration.aboutGroupName).trim().isEmpty();

			if (matcher.group(SemanticDuration.pattern1GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticDuration.p1Unit1) != null)
					unit = EDurationUnits.valueOf(mapVariation(matcher.group(SemanticDuration.p1Unit1).toLowerCase()));
				if (matcher.group(SemanticDuration.p1Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticDuration.p1Numbers1));
				if (matcher.group(SemanticDuration.p1Numbers2) != null) {
					EDurationUnits unit_;
					if (matcher.group(SemanticDuration.p1Unit2) != null) {
						unit_ = EDurationUnits
								.valueOf(mapVariation(matcher.group(SemanticDuration.p1Unit2).toLowerCase()));
					} else {
						unit_ = unit;
					}
					double toValue_ = SemanticDuration
							.convertValue(Double.valueOf(matcher.group(SemanticDuration.p1Numbers2)), unit_, unit);
			...
			} else if (matcher.group(SemanticDuration.pattern2GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticDuration.p2Unit1) != null)
					unit = EDurationUnits.valueOf(mapVariation(matcher.group(SemanticDuration.p2Unit1).toLowerCase()));
				if (matcher.group(SemanticDuration.p2Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticDuration.p2Numbers1));
				if (matcher.group(SemanticDuration.p2Numbers2) != null) {
					double toValue_ = Double.valueOf(matcher.group(SemanticDuration.p2Numbers2));
					/*
					 * Since there are only positive weights we either set the
					 * variance and mean or from to values.
					 */
			...
			} else if (matcher.group(SemanticDuration.pattern3GrouName) != null) {
				if (matcher.group(SemanticDuration.p3Unit1) != null)
					unit = EDurationUnits.valueOf(mapVariation(matcher.group(SemanticDuration.p3Unit1).toLowerCase()));
				if (matcher.group(SemanticDuration.p3Numbers1) != null)
					meanValue = Double.valueOf(matcher.group(SemanticDuration.p3Numbers1));
			}
			return this;
		}
