...
//				continue;
			...
			List<IInjury> rawData = new ArrayList<>(new AnnoDBRDFReader(IInjury.class, rdfFile, annotationFile).get());

			final Map<Class<? extends IOBIEThing>, List<IOBIEThing>> results = new HashMap<>();

			results.put(IInjury.class, new ArrayList<>());

			for (IInjury injury : rawData) {

				System.out.println(OBIEClassFormatter.format(injury));
				results.get(IInjury.class).add(injury);
