...
public class SemanticFrequency extends AbstractNumericInterpreter {
	/**
	 * 
	 */
	...
	static public enum EFrequencyUnits implements ISingleUnit {
		...
		private EFrequencyUnits(double factor) {
			this.factor = factor;
		}

		public static EFrequencyUnits getDefault() {
			return daily;
		}
	...
	final static String numbers_ = writtenNumbers + "|(once|twice|thrice)";
	final static String freeSpace_ = "[^\\d\\w\\.,]";
	final static String freeSpaceQuestionMark_ = freeSpace_ + "?";
	final static String unit_ = "(daily|weekly|(per|a)\\Wday|times)(?!" + freeSpaceQuestionMark_ + "/)";
	...
	final public EFrequencyUnits unit;
	final public double meanValue;
	final public boolean about;
	final public double fromValue;
	final public double toValue;

	private SemanticFrequency(String surfaceForm, EFrequencyUnits unit, double meanValue, boolean about,
	...
	public SemanticFrequency convertTo(EFrequencyUnits toWeightUnit) {
		return new SemanticFrequency(surfaceForm, toWeightUnit, convertValue(meanValue, toWeightUnit), about,
				convertValue(fromValue, toWeightUnit), convertValue(toValue, toWeightUnit));
	}

	public static double convertValue(double value, EFrequencyUnits fromWeightUnit, EFrequencyUnits toWeightUnit) {
		return (value * fromWeightUnit.factor) / toWeightUnit.factor;
	}

	private double convertValue(double value, EFrequencyUnits toWeightUnit) {
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SemanticFrequency other = (SemanticFrequency) obj;
	...
	public SemanticFrequency normalize() {
		return convertTo(EFrequencyUnits.getDefault());
	}
	...
	public static class Builder extends AbstractInterpreterBuilder {
		...
		final public static EFrequencyUnits defaultUnit = EFrequencyUnits.undef;
	...
	((~|\babout[^\d\w\.,])?)(((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))?)|((((\d+(\.|,)\d+)|\d+))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?)))|((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))))\b
		...
		private EFrequencyUnits unit = defaultUnit;
		...
		public EFrequencyUnits getUnit() {
			return unit;
		}

		public Builder setUnit(EFrequencyUnits unit) {
			this.unit = unit;
			return this;
		}
		...
		public SemanticFrequency build() {
			...
			return new SemanticFrequency(surfaceForm, unit, _meanValue, about, _fromValue, _toValue);
		}

		public Builder interprete(final String surfaceForm) {
			Matcher matcher = SemanticFrequency.PATTERN.matcher(surfaceForm);
			if (!matcher.find())
				return this;
		...
		public Builder fromMatcher(final Matcher matcher) {
			...
			about = matcher.group(SemanticFrequency.aboutGroupName) != null
					&& !matcher.group(SemanticFrequency.aboutGroupName).trim().isEmpty();

			if (matcher.group(SemanticFrequency.pattern1GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticFrequency.p1Unit1) != null)
					unit = EFrequencyUnits
							.valueOf(mapVariation(matcher.group(SemanticFrequency.p1Unit1).toLowerCase()));
				if (matcher.group(SemanticFrequency.p1Numbers1) != null)
					fromValue_ = Double.valueOf(mapWrittenNumbertoInt(matcher.group(SemanticFrequency.p1Numbers1)));
				if (matcher.group(SemanticFrequency.p1Numbers2) != null) {
					EFrequencyUnits unit_;
					if (matcher.group(SemanticFrequency.p1Unit2) != null) {
						unit_ = EFrequencyUnits
								.valueOf(mapVariation(matcher.group(SemanticFrequency.p1Unit2).toLowerCase()));
					} else {
						unit_ = unit;
					}
					double toValue_ = SemanticFrequency.convertValue(
							Double.valueOf(mapWrittenNumbertoInt(matcher.group(SemanticFrequency.p1Numbers2))), unit_,
							unit);
			...
			} else if (matcher.group(SemanticFrequency.pattern2GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticFrequency.p2Unit1) != null)
					unit = EFrequencyUnits
							.valueOf(mapVariation(matcher.group(SemanticFrequency.p2Unit1).toLowerCase()));
				if (matcher.group(SemanticFrequency.p2Numbers1) != null)
					fromValue_ = Double.valueOf(mapWrittenNumbertoInt(matcher.group(SemanticFrequency.p2Numbers1)));
				if (matcher.group(SemanticFrequency.p2Numbers2) != null) {
					double toValue_ = Double
							.valueOf(mapWrittenNumbertoInt(matcher.group(SemanticFrequency.p2Numbers2)));
					/*
					 * Since there are only positive weights we either set the
					 * variance and mean or from to values.
					 */
			...
			} else if (matcher.group(SemanticFrequency.pattern3GrouName) != null) {
				if (matcher.group(SemanticFrequency.p3Unit1) != null)
					unit = EFrequencyUnits
							.valueOf(mapVariation(matcher.group(SemanticFrequency.p3Unit1).toLowerCase()));
				if (matcher.group(SemanticFrequency.p3Numbers1) != null)
					meanValue = Double.valueOf(mapWrittenNumbertoInt(matcher.group(SemanticFrequency.p3Numbers1)));
			}
			return this;
		}
	}
