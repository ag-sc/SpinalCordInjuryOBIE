...
import de.uni.bielefeld.sc.hterhors.psink.obie.projects.scio.ontology.interfaces.IEvent;
 ...
 *         Dec 14, 2016
 */
public class ExcelCorpusConverter {

	public static void main(String[] args) throws Exception {

		System.out.println("#####START#####");
		ExcelCorpusConverter e = new ExcelCorpusConverter();
	 ...
	 * Settings
	 */
	private static final String RESOURCE_FILE = "excel/Access_DB_Daten_TSV.csv";

	public ExcelCorpusConverter() throws Exception {
		...
		for (Entry<String, List<IResult>> dp : excelCorpus.entrySet()) {
			...
			for (IResult r : dp.getValue()) {
				...
				System.out.println(ToStringFormatter.formatString(r.toString()));
	...
	private void writeModel(Model model, Lang l) throws Exception {
		RDFDataMgr.write(
				new BufferedOutputStream(
						new FileOutputStream(new File("excel/FullExcel2RDFv" + 62 + "." + l.getName().toLowerCase()))),
				model, l);
	...
	private IResult buildResult(String[] line) {
		...
		tp.setEventBefore(((IEvent) ((IExperimentalGroup) r.getTargetExperimentalGroup()).getInjuryModel()));
		tp.setDuration(reflectClassByName(line[TIMEPOINT], null, IDuration.class));
		tp.setEventAfter(((IEvent) ((IExperimentalGroup) r.getTargetExperimentalGroup()).getTreatmentTypes().get(0)));

		Observation o2 = new Observation().setBelongsToExperimentalGroup(buildExperimentalTargetGroup(line))
	...
	private <I extends ISCIOThing, C extends I> I reflectClassByName(String className, C defaultInstance,
		...
		try {
			id = Class.forName(
					...
					.newInstance();
			Field annID = id.getClass().getDeclaredField("annotationID");
			annID.setAccessible(true);
			annID.set(id, annotationID);
