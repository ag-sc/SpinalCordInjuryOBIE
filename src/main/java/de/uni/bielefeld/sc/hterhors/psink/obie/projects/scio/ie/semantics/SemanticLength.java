...
public class SemanticLength extends AbstractNumericInterpreter implements IDatatypeInterpretation {
	/**
	 * 
	 */
	...
	private SemanticLength(String surfaceForm, ELengthUnits unit, double meanValue, boolean about, double fromValue,
			double toValue) {
		super(surfaceForm);
		this.unit = unit;
	...
	public SemanticLength convertTo(ELengthUnits toWeightUnit) {
		return new SemanticLength(surfaceForm, toWeightUnit, convertValue(meanValue, toWeightUnit), about,
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SemanticLength other = (SemanticLength) obj;
	...
	public SemanticLength normalize() {
	...
	((~|\babout[^\d\w\.,])?)(((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))?)|((((\d+(\.|,)\d+)|\d+))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?)))|((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))))\b
		...
		private ELengthUnits unit = defaultUnit;
		...
		public ELengthUnits getUnit() {
			return unit;
		}

		public Builder setUnit(ELengthUnits unit) {
			this.unit = unit;
			return this;
		}
		...
		public SemanticLength build() {
			...
			return new SemanticLength(surfaceForm, unit, _meanValue, about, _fromValue, _toValue);
		}

		public Builder interprete(final String surfaceForm) {

			Matcher matcher = SemanticLength.PATTERN.matcher(surfaceForm);
		...
		public Builder fromMatcher(final Matcher matcher) {
			...
			about = matcher.group(SemanticLength.aboutGroupName) != null
					&& !matcher.group(SemanticLength.aboutGroupName).trim().isEmpty();

			if (matcher.group(SemanticLength.pattern1GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticLength.p1Unit1) != null)
					unit = ELengthUnits.valueOf(mapUnit(matcher.group(SemanticLength.p1Unit1).toLowerCase()));
				if (matcher.group(SemanticLength.p1Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticLength.p1Numbers1));
				if (matcher.group(SemanticLength.p1Numbers2) != null) {
					ELengthUnits unit_;
					if (matcher.group(SemanticLength.p1Unit2) != null) {
						unit_ = ELengthUnits.valueOf(mapUnit(matcher.group(SemanticLength.p1Unit2).toLowerCase()));
					} else {
						unit_ = unit;
					}
					double toValue_ = SemanticLength
							.convertValue(Double.valueOf(matcher.group(SemanticLength.p1Numbers2)), unit_, unit);
			...
			} else if (matcher.group(SemanticLength.pattern2GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticLength.p2Unit1) != null)
					unit = ELengthUnits.valueOf(mapUnit(matcher.group(SemanticLength.p2Unit1).toLowerCase()));
				if (matcher.group(SemanticLength.p2Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticLength.p2Numbers1));
				if (matcher.group(SemanticLength.p2Numbers2) != null) {
					double toValue_ = Double.valueOf(matcher.group(SemanticLength.p2Numbers2));
					/*
					 * Since there are only positive weights we either set the
					 * variance and mean or from to values.
					 */
			...
			} else if (matcher.group(SemanticLength.pattern3GrouName) != null) {
				if (matcher.group(SemanticLength.p3Unit1) != null)
					unit = ELengthUnits.valueOf(mapUnit(matcher.group(SemanticLength.p3Unit1).toLowerCase()));
				if (matcher.group(SemanticLength.p3Numbers1) != null)
					meanValue = Double.valueOf(matcher.group(SemanticLength.p3Numbers1));
			}
			return this;
		}
