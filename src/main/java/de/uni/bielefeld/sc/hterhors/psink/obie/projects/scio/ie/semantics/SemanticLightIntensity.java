...
public class SemanticLightIntensity extends AbstractNumericInterpreter {
	/**
	 * 
	 */
	...
	private SemanticLightIntensity(String surfaceForm, ELightIntensityUnits unit, double meanValue, boolean about,
	...
	public SemanticLightIntensity convertTo(ELightIntensityUnits toWeightUnit) {
		return new SemanticLightIntensity(surfaceForm, toWeightUnit, convertValue(meanValue, toWeightUnit), about,
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SemanticLightIntensity other = (SemanticLightIntensity) obj;
	...
	public SemanticLightIntensity normalize() {
	...
	((~|\babout[^\d\w\.,])?)(((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))?)|((((\d+(\.|,)\d+)|\d+))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?)))|((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))))\b
		...
		private ELightIntensityUnits unit = defaultUnit;
		...
		public ELightIntensityUnits getUnit() {
			return unit;
		}

		public Builder setUnit(ELightIntensityUnits unit) {
			this.unit = unit;
			return this;
		}
		...
		public SemanticLightIntensity build() {
			...
			return new SemanticLightIntensity(surfaceForm, unit, _meanValue, about, _fromValue, _toValue);
		}

		public Builder interprete(final String surfaceForm) {
			Matcher matcher = SemanticLightIntensity.PATTERN.matcher(surfaceForm);
			if (!matcher.find())
				return this;
		...
		public Builder fromMatcher(final Matcher matcher) {
			...
			about = matcher.group(SemanticLightIntensity.aboutGroupName) != null
					&& !matcher.group(SemanticLightIntensity.aboutGroupName).trim().isEmpty();

			if (matcher.group(SemanticLightIntensity.pattern1GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticLightIntensity.p1Unit1) != null)
					unit = ELightIntensityUnits
							.valueOf(mapVariation(matcher.group(SemanticLightIntensity.p1Unit1).toLowerCase()));
				if (matcher.group(SemanticLightIntensity.p1Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticLightIntensity.p1Numbers1));
				if (matcher.group(SemanticLightIntensity.p1Numbers2) != null) {
					ELightIntensityUnits unit_;
					if (matcher.group(SemanticLightIntensity.p1Unit2) != null) {
						unit_ = ELightIntensityUnits
								.valueOf(mapVariation(matcher.group(SemanticLightIntensity.p1Unit2).toLowerCase()));
					} else {
						unit_ = unit;
					}
					double toValue_ = SemanticLightIntensity.convertValue(
							Double.valueOf(matcher.group(SemanticLightIntensity.p1Numbers2)), unit_, unit);
			...
			} else if (matcher.group(SemanticLightIntensity.pattern2GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticLightIntensity.p2Unit1) != null)
					unit = ELightIntensityUnits
							.valueOf(mapVariation(matcher.group(SemanticLightIntensity.p2Unit1).toLowerCase()));
				if (matcher.group(SemanticLightIntensity.p2Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticLightIntensity.p2Numbers1));
				if (matcher.group(SemanticLightIntensity.p2Numbers2) != null) {
					double toValue_ = Double.valueOf(matcher.group(SemanticLightIntensity.p2Numbers2));
					/*
					 * Since there are only positive weights we either set the
					 * variance and mean or from to values.
					 */
			...
			} else if (matcher.group(SemanticLightIntensity.pattern3GrouName) != null) {
				if (matcher.group(SemanticLightIntensity.p3Unit1) != null)
					unit = ELightIntensityUnits
							.valueOf(mapVariation(matcher.group(SemanticLightIntensity.p3Unit1).toLowerCase()));
				if (matcher.group(SemanticLightIntensity.p3Numbers1) != null)
					meanValue = Double.valueOf(matcher.group(SemanticLightIntensity.p3Numbers1));
			}
			return this;
		}
	}
