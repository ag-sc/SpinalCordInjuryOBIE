...
public class SemanticDepth extends AbstractNumericInterpreter implements IDatatypeInterpretation {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	static public enum EDepthUnits implements ISingleUnit {

		undef(0), µm(0.001d), mm(1 / 10d), cm(1d);

		final public double factor;

		private EDepthUnits(double factor) {
			this.factor = factor;
		}

		public static EDepthUnits getDefault() {
			return cm;
		}
	...
	final public EDepthUnits unit;
	final public double meanValue;
	final public boolean about;
	final public double fromValue;
	final public double toValue;

	private SemanticDepth(String surfaceForm, EDepthUnits unit, double meanValue, boolean about, double fromValue,
			double toValue) {
		super(surfaceForm);
		this.unit = unit;
	...
	public SemanticDepth convertTo(EDepthUnits toWeightUnit) {
		return new SemanticDepth(surfaceForm, toWeightUnit, convertValue(meanValue, toWeightUnit), about,
				convertValue(fromValue, toWeightUnit), convertValue(toValue, toWeightUnit));
	}

	public static double convertValue(double value, EDepthUnits fromWeightUnit, EDepthUnits toWeightUnit) {
		return (value * fromWeightUnit.factor) / toWeightUnit.factor;
	}

	private double convertValue(double value, EDepthUnits toWeightUnit) {
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SemanticDepth other = (SemanticDepth) obj;
	...
	public String toString() {
		return "SemanticDepth[surfaceForm=" + surfaceForm + ", unit=" + unit + ", meanValue=" + meanValue + ", about="
				+ about + ", fromValue=" + fromValue + ", toValue=" + toValue + "]";
	}

	public SemanticDepth normalize() {
		return convertTo(EDepthUnits.getDefault());
	}
	...
	public static class Builder extends AbstractInterpreterBuilder {
		...
		final public static EDepthUnits defaultUnit = EDepthUnits.undef;
	...
	((~|\babout[^\d\w\.,])?)(((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))?)|((((\d+(\.|,)\d+)|\d+))([^\d\w\.,](to|and)[^\d\w\.,]|[^\d\w\.,]?((\+?-)|±|[^\d\w\.,])[^\d\w\.,]?)(((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?)))|((((\d+(\.|,)\d+)|\d+))[^\d\w\.,]?(((((m)(milli)?|((k)(ilo)?)))?(g)(ram)?))))\b
		...
		private EDepthUnits unit = defaultUnit;
		...
		public EDepthUnits getUnit() {
			return unit;
		}

		public Builder setUnit(EDepthUnits unit) {
			this.unit = unit;
			return this;
		}
		...
		public SemanticDepth build() {
			...
			return new SemanticDepth(surfaceForm, unit, _meanValue, about, _fromValue, _toValue);
		}

		public Builder interprete(final String surfaceForm) {

			Matcher matcher = SemanticDepth.PATTERN.matcher(surfaceForm);
		...
		public Builder fromMatcher(final Matcher matcher) {
			...
			about = matcher.group(SemanticDepth.aboutGroupName) != null
					&& !matcher.group(SemanticDepth.aboutGroupName).trim().isEmpty();

			if (matcher.group(SemanticDepth.pattern1GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticDepth.p1Unit1) != null)
					unit = EDepthUnits.valueOf(mapUnit(matcher.group(SemanticDepth.p1Unit1).toLowerCase()));
				if (matcher.group(SemanticDepth.p1Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticDepth.p1Numbers1));
				if (matcher.group(SemanticDepth.p1Numbers2) != null) {
					EDepthUnits unit_;
					if (matcher.group(SemanticDepth.p1Unit2) != null) {
						unit_ = EDepthUnits.valueOf(mapUnit(matcher.group(SemanticDepth.p1Unit2).toLowerCase()));
					} else {
						unit_ = unit;
					}
					double toValue_ = SemanticDepth
							.convertValue(Double.valueOf(matcher.group(SemanticDepth.p1Numbers2)), unit_, unit);
			...
			} else if (matcher.group(SemanticDepth.pattern2GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticDepth.p2Unit1) != null)
					unit = EDepthUnits.valueOf(mapUnit(matcher.group(SemanticDepth.p2Unit1).toLowerCase()));
				if (matcher.group(SemanticDepth.p2Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticDepth.p2Numbers1));
				if (matcher.group(SemanticDepth.p2Numbers2) != null) {
					double toValue_ = Double.valueOf(matcher.group(SemanticDepth.p2Numbers2));
					/*
					 * Since there are only positive weights we either set the
					 * variance and mean or from to values.
					 */
			...
			} else if (matcher.group(SemanticDepth.pattern3GrouName) != null) {
				if (matcher.group(SemanticDepth.p3Unit1) != null)
					unit = EDepthUnits.valueOf(mapUnit(matcher.group(SemanticDepth.p3Unit1).toLowerCase()));
				if (matcher.group(SemanticDepth.p3Numbers1) != null)
					meanValue = Double.valueOf(matcher.group(SemanticDepth.p3Numbers1));
			}
			return this;
		}
