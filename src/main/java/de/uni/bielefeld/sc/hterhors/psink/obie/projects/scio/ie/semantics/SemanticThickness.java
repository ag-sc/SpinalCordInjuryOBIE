...
public class SemanticThickness extends AbstractNumericInterpreter implements IDatatypeInterpretation {
	/**
	 * 
	 */
	...
	private SemanticThickness(String surfaceForm, EThicknessUnits unit, double meanValue, boolean about,
	...
	public SemanticThickness convertTo(EThicknessUnits toWeightUnit) {
		return new SemanticThickness(surfaceForm, toWeightUnit, convertValue(meanValue, toWeightUnit), about,
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SemanticThickness other = (SemanticThickness) obj;
	...
	public SemanticThickness normalize() {
	...
	public static class Builder extends AbstractInterpreterBuilder {
		...
		private EThicknessUnits unit = defaultUnit;
		...
		public Builder setUnit(EThicknessUnits unit) {
			this.unit = unit;
			return this;
		}
		...
		public SemanticThickness build() {
			...
			return new SemanticThickness(surfaceForm, unit, _meanValue, about, _fromValue, _toValue);
		}

		public Builder interprete(final String surfaceForm) {

			Matcher matcher = SemanticThickness.PATTERN.matcher(surfaceForm);
		...
		public Builder fromMatcher(final Matcher matcher) {
			...
			about = matcher.group(SemanticThickness.aboutGroupName) != null
					&& !matcher.group(SemanticThickness.aboutGroupName).trim().isEmpty();

			if (matcher.group(SemanticThickness.pattern1GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticThickness.p1Unit1) != null)
					unit = EThicknessUnits.valueOf(mapVariation(matcher.group(SemanticThickness.p1Unit1).toLowerCase()));
				if (matcher.group(SemanticThickness.p1Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticThickness.p1Numbers1));
				if (matcher.group(SemanticThickness.p1Numbers2) != null) {
					EThicknessUnits unit_;
					if (matcher.group(SemanticThickness.p1Unit2) != null) {
						unit_ = EThicknessUnits
								.valueOf(mapVariation(matcher.group(SemanticThickness.p1Unit2).toLowerCase()));
					} else {
						unit_ = unit;
					}
					double toValue_ = SemanticThickness
							.convertValue(Double.valueOf(matcher.group(SemanticThickness.p1Numbers2)), unit_, unit);
			...
			} else if (matcher.group(SemanticThickness.pattern2GrouName) != null) {
				double fromValue_ = defaultFromValue;
				if (matcher.group(SemanticThickness.p2Unit1) != null)
					unit = EThicknessUnits.valueOf(mapVariation(matcher.group(SemanticThickness.p2Unit1).toLowerCase()));
				if (matcher.group(SemanticThickness.p2Numbers1) != null)
					fromValue_ = Double.valueOf(matcher.group(SemanticThickness.p2Numbers1));
				if (matcher.group(SemanticThickness.p2Numbers2) != null) {
					double toValue_ = Double.valueOf(matcher.group(SemanticThickness.p2Numbers2));
					/*
					 * Since there are only positive weights we either set the
					 * variance and mean or from to values.
					 */
			...
			} else if (matcher.group(SemanticThickness.pattern3GrouName) != null) {
				if (matcher.group(SemanticThickness.p3Unit1) != null)
					unit = EThicknessUnits.valueOf(mapVariation(matcher.group(SemanticThickness.p3Unit1).toLowerCase()));
				if (matcher.group(SemanticThickness.p3Numbers1) != null)
					meanValue = Double.valueOf(matcher.group(SemanticThickness.p3Numbers1));
			}
			return this;
		}
